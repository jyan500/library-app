...
const MultiCardCarousel = ({ itemsPerPage }: Props) => {
    ...
    const handlePrev = () => {
        /* 
        if clicking previous on index 0, calculate the last page by figuring out the corresponding index
        on the circle pages indicator. For example if itemsPerPage = 3, and the total amount of items is 7,
        this should result in a currentIndex value of 6 in order to correspond with the circle pages indicator.

        The circle pages indicator shows the current page based on currentIndex === i * (itemsPerPage), therefore
        Math.floor(7/3) is 2, and 2 * 3 = 6
        6 === 2 * 3, so this currentIndex value would corespond to the proper indicator. 

        TODO: need to fix a bug when items.length % itemsPerPage === 0, the previous button will land on the wrong index 
        causing inaccuracy on the page indicator

        */
        let newIndex = 0
        if (itemsPerPage > 1){
            newIndex = currentIndex === 0 ? (items.length % itemsPerPage === 0 ? Math.floor(items.length/itemsPerPage)*itemsPerPage : (Math.floor(items.length/itemsPerPage)*itemsPerPage)-1) : currentIndex - itemsPerPage
        }
        else {
    ...
    const handleNext = () => {
        let newIndex = 0 
        if (itemsPerPage > 1){
            newIndex = currentIndex + itemsPerPage >= items.length ? 0 : currentIndex + itemsPerPage
        }
        else {
                    ...
                    */}
                    <div className="tw-flex tw-transition-transform tw-duration-700 tw-ease-in-out" style={{ transform: `translateX(-${(currentIndex/itemsPerPage) * 100}%)` }}>
                        {items.map((item, index) => (
                            ...
                            */
                            <div key={item.id} style = {style} className = "tw-w-full tw-h-full tw-flex-shrink-0 tw-px-2">
